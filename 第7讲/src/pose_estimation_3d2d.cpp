...
// g2o求解与ceres求解不能同时打开，有两个头文件不知道为什么同时打开会报错
  ...
  Sophus::SE3d &pose)
{
  ...
  for (int i=0; i <N; i++){                    // 循环添加所有残差
    ...
    Eigen::Vector3d xyz(points_3d[i][0],points_3d[i][1],points_3d[i][2]);
    Eigen::Vector2d uv(points_2d[i][0],points_2d[i][1]);

    problem.AddResidualBlock(                                 // 残差块
      new POSE_FITTING_COST(xyz, uv, K_eigen),         // CostFunction *cost_function, 
      nullptr,                                                  // LossFunction *loss_function,   
      rt                                                    // const vector<double *> parameter_blocks 、或者 double *x0, double *x1, ...
    );
  }
  ceres::LocalParameterization *local_param = new SE3dceres();
  problem.SetParameterization(rt, local_param);
  ...
  Sophus::SE3d poseByCeres = Sophus::SE3d::exp(Vector6d(rt));
  cout << "poseByCeres T = \n" ;
  cout << poseByCeres.matrix() << endl;
