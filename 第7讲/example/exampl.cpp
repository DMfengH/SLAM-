...
Matrix6d JRInv(const Sophus::SE3d &e){
    Matrix6d J;
    J.block(0,0,3,3) = Sophus::SO3d::hat(e.so3().log());    // 旋转部分的反对称矩阵
    J.block(0,3,3,3) = Sophus::SO3d::hat(e.translation());  // 平移部分的反对称矩阵
    J.block(3,0,3,3) = Matrix3d::Zero(3,3);
    J.block(3,3,3,3) = Sophus::SO3d::hat(e.so3().log());    // 旋转部分的反对称矩阵
...
//        residual = sqrt_info * ((measurment_se3.inverse() * estimate).log());
        ...
        if(jacobians){
            if(jacobians[0]) {
                ...
                jacobian_i = (-J * pose_j.inverse().Adj()).transpose();
            }

            if(jacobians[1]){
                Eigen::Map<Matrix6d> jacobian_j(jacobians[1]);
                Matrix6d J = JRInv(Sophus::SE3d::exp(residual));
                jacobian_j = (J * pose_j.inverse().Adj()).transpose();
            }
        }
        return true;
    }
...
//    auto tmp = Sophus::SE3d(q,t).log();
//    auto tmp1 = Sophus::SE3d::exp(tmp);
    for(int i = 0; i < 6; i++){
        _p.se3[i] = se(i, 0);
    }
}
